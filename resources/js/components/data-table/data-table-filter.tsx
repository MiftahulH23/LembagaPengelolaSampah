...
function FilterItem<T>(props: FilterComponent<T>) {
    ...
    if (state === column.id) {
        return <FilterComponent column={column} data={data} label={label} />;
    }

    return (
        <Button size="sm" variant="ghost" className="h-7 justify-start !ps-1 text-sm" onClick={() => setState(column.id as keyof T)}>
            <Icon className="size-4 text-muted-foreground" />
            <span className="capitalize">{label ?? filterText[filterKey]}</span>

            {isFiltering && (
                <span className="ms-auto -me-1 line-clamp-1 inline-flex h-5 max-h-full max-w-18 items-center rounded px-1 text-xs font-[weight:inherit] text-muted-foreground capitalize">
...
function GroupedFilter<T>(props: DataTableFilter<T>) {
    ...
    if (standalone && standaloneColumn) {
        ...
        return <FilterComponent column={standaloneColumn!} data={data} label={label} className={className} standalone {...rest} />;
    }
        ...
        >
            <PopoverTrigger asChild>
                <Button variant="outline" className={className} {...rest}>
                    ...
                    <span className="sr-only text-foreground @2xl:not-sr-only">Filter</span>
                </Button>
            </PopoverTrigger>
            <PopoverContent align="center" className="group flex !w-max !min-w-52 flex-col !gap-1 p-2">
                {state === 'idle' && (
                    <>
                        <div className="flex items-center justify-between px-1 pt-0.5">
                            <div className="text-xs text-foreground/90">Pilih Filter</div>
                            <button
                                onClick={resetColumnsFilter}
                                disabled={!isFiltering}
                                className="text-xs text-muted-foreground underline-offset-2 not-disabled:cursor-pointer not-disabled:hover:text-foreground not-disabled:hover:underline"
                            >
                                Reset
                            </button>
                        </div>
                ...
                {columns.length > 0 &&
                    columns.map((column, i) => {
                        ...
                        return (
                            data &&
                            data.length > 0 && <FilterItem key={i} column={column} data={data} label={extendItems(column)?.label ?? undefined} />
                        );
                    })}
            </PopoverContent>
        </Popover>
